...
const VideoList = ({ video, onVideoClick, youtubeService }) => {
  ...
  useEffect(() => {
    id && youtubeService.statistics(id).then((items) => items.map((item) => setStatistics(item.statistics)));
    // videoId && youtubeService.statistics(videoId).then((items) => items.map((item) => console.log(item.statistics)));
  }, [youtubeService, id]);

  return (
    // 컨텐트 오버시 메뉴 모양 및 영상 시간, 두개 버튼 나오게끔 (포지션 사용)
    <div className={styles.listWrap} onClick={() => onVideoClick({ ...video, statistics })}>
